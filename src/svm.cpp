...
string snames[18] = {"push", "pop", "dup", "swap", "add", "sub", "mult", "div", "goto", "jmpeq", "jmpgt", "jmpge",
                     "jmplt", "jmple", "skip", "store", "load", "print"};
    ...
    if (itype == Instruction::IPOP || itype == Instruction::IDUP || itype == Instruction::IPRINT ||
        ...
        switch (itype) {
            case (Instruction::IPUSH):
                opstack.push(instr->argint);
                break;
            case (Instruction::ISTORE):
                if (opstack.empty()) perror("Can't store from an empty stack");
                register_write(instr->argint, opstack.top());
                opstack.pop();
                break;
                break;
            case (Instruction::ILOAD):
                opstack.push(register_read(instr->argint));
                break;
                break;
            default:
                perror("Programming Error 2");
        }
        pc++;
    }
